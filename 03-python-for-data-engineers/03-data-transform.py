...
file_location = './data/sample_data.csv'
...
for row in unique_data:
    if not row["Age"]:
        print(f'Customer {row["Customer_Name"]} does not have an age value')
        row["Age"] = 0
...
cleaned_data = [row for row in unique_data if int(row["Age"]) <= 100 and float(row["Purchase_Amount"]) <= 1000]
...
for row in cleaned_data:
    # print(row["Gender"], row['Purchase_Amount'])
...
duckdb_con = duckdb.connect("duck.db")
...
con.execute(
    "CREATE TABLE data_outliers AS SELECT * FROM data_cleaned_missing WHERE Age <= 100 AND Purchase_Amount <=1000"
)

# Question: How do you convert the Gender column to a binary format (0 for Female, 1 for Male) in DuckDB?
con.execute(
    "CREATE TABLE data_cleaned_gender AS SELECT *, CASE WHEN Gender = 'Female' THEN 0 ELSE 1 END AS Gender_Binary FROM data_outliers"
)
...
total_purchase_by_gender = con.execute("SELECT Gender_Binary, SUM(Purchase_Amount) FROM data_cleaned GROUP BY Gender_Binary").fetchall()
